...
import { CreateAssetParams, CreateTransferAssetParams } from 'blockin-test-package';
import { IChainDriver, UniversalTxn } from 'blockin-test-package';
...
export class AlgoDriver implements IChainDriver {
    ...
    /**
     * Since we are using a 0 Algos payment Txn due to Pera Wallet not supporting bytes signatures
     * natively, we use the 'note' field of the transaction to include the challenge. This function
     * parses the txn bytes and returns only the challenge string (ignores all other txn info).
     */
